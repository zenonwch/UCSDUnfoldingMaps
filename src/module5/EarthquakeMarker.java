...
public abstract class EarthquakeMarker extends CommonMarker {
    ...
    public void drawMarker(final PGraphics pg, final float x, final float y) {
        ...
        final String age = getStringProperty("age");
    ...
    public void showTitle(final PGraphics pg, final float x, final float y) {
        ...
        final String text = getTitle();
        final float w = pg.textWidth(text);
        final float padding = 3;

        pg.pushStyle();
        pg.fill(240, 200);
        pg.rect(x + 2 * radius, y, w + 2 * padding, 18);
        pg.fill(20);
        pg.textSize(12);
        pg.textAlign(PConstants.LEFT, PConstants.TOP);
        pg.text(text, x + 2 * radius + padding, y);
        pg.popStyle();
    }
    ...
    public double threatCircle() {
        final double miles = 20.0f * Math.pow(1.8, 2 * getMagnitude() - 5);
        final double km = (miles * kmPerMile);
        return km;
    }
