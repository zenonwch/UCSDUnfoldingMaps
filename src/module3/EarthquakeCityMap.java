...
//			map = new UnfoldingMap(this, 200, 50, 700, 500, new Google.GoogleMapProvider());
        ...
        if (earthquakes.size() > 0) {
            final PointFeature f = earthquakes.get(0);
            System.out.println(f.getProperties());
            final Object magObj = f.getProperty("magnitude");
        ...
        for (final PointFeature f : earthquakes) {
            final SimplePointMarker marker = createMarker(f);
            final Float mag = Float.parseFloat(f.getProperty("magnitude").toString());
            styleMarker(marker, mag);
            markers.add(marker);
        }
    ...
    private void styleMarker(final SimplePointMarker marker, final Float mag) {
